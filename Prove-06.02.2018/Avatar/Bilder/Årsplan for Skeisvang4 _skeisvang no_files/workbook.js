...
function quizstats(ttype,using,ignoring) {
    var course,group,config,basetemalist, personal;
    ...
    var canonical = false;
    personal = localStorage.getItem("stats_" + wbinfo.coursename);
    if (personal) {
      try {
    ...
    basetemalist = _.union(basetemalist,personal);     // join base list with localstorage
    var teachid = 0;
    ...
    if (course && group) {
        ...
        function showstats(res,using,ignoring) {
            ...
            $j("#saveconf").click(function() {
                       var personal = _.difference(using,basetemalist);
                       localStorage.setItem("stats_" + wbinfo.coursename, JSON.stringify(personal));
                        $j("#saveconf").hide();
                    });
            function showsave() {
                $j("#saveconf").hide();
...
function showResults(group,container,contopt) {
    ...
    var klassScore = 0;   // calc average score for class
    var klassCount = 0;
    ...
    function _showresults(field,dir) {
       ...
       var avgScore =  klassCount ? (klassScore / klassCount) : 0;
       display += '<div class="userres"><span class="numb"> &nbsp; </span>'
                + '<span class="fn">  &nbsp; </span>'
                + '<span class="ln">Average score</span>'
                + '<span class="kara">' + avgScore.toFixed(2) + '</span>'
                + ((wbinfo.courseinfo.contopt.karak == 1) ? '<span class="kara">karakter ' + score2grade(avgScore,skala) + '</span>' : '')
                + '</div>';
...
function renderPage() {
  ...
  $j.getJSON(mybase+'/log/getqcon',{ container:wbinfo.containerid }, function(container) {
    ...
    contopt = {};
    if (courseinfo.contopt && container.qtype === 'quiz') {
    ...
    wbinfo.page[wbinfo.containerid] = _.isFinite(wbinfo.page[wbinfo.containerid]) ? wbinfo.page[wbinfo.containerid] : 0;
        ...
        if (showlist.length) {
          wb.render[wbinfo.layout].qlist(wbinfo.containerid, showlist, contopt, function(renderq) {
                ...
                if (contopt.omstart && contopt.omstart == "1") {
                    ...
                    $j("#qlistbox").undelegate(".reloadme(","click");
                    $j("#qlistbox").delegate(".reloadme","click", function() {
                        var myid = $j(this).parent().parent().attr("id");
                        if (myid == undefined) {
                        ...
                        var qid = elm[0];
                        var iid = elm[1];
                        $j.post(mybase+"/log/studResetAnswer",{ cid:wbinfo.containerid,qid:qid,iid:iid},function(res) {
                            // redraw the question with new parameters without disturbing other questions
                            // only chosen question is regenerated and redrawn in-situ
                            wb.render[wbinfo.layout].qrend(contopt,iid,qid,res.qua,renderq.qrender,renderq.scorelist,function(adjust) {
                                $j("#"+adjust.sscore.qdivid).html(adjust.sscore.qdiv);
                                redrawQuestion(iid,0,0);  // redraw next question if any
                            });
                        });
                    });
                }
...
function edqlist() {
  ...
  var s = '<div id="wbmain">' + head + '<div id="qlistbox">'
         +showqlist
         ...
         + '<div title="Legg til eksisterende sprsml" id="attach" class="gradebutton">attach</div>'
         + '<div title="Legg til eksisterende quiz" id="attachq" class="gradebutton">addQuiz</div>'
         + '</div></div>';
  $j("#main").html(s);
  ...
  $j("#qlist").dialog({ width:550, height:500, autoOpen:false, title:'Pick question',
     buttons: {
       ...
       "Oppdater": function() {
               var master = $j("#master").is(":checked");
               $j( this ).dialog( "close" );
               ...
               var action = master ? 'master' : 'insert';
               $j.post(mybase+'/log/editqncontainer', { action:action, container:wbinfo.containerid, nuqs:nufilter.join(',') }, function(resp) {
  ...
  function selectQuestQuiz(qtlist) {
    ...
    var sensei = {};   // list of old master copies (so we only show the first copy of a master)
    ...
    if (qtlist ) {
      ...
      for(var tname in qtlist) {
        ...
        for(var i in qtlist[tname][userinfo.id]) {
          ...
          try {
            oparam = JSON.parse(qqa.otxt);
          }
          catch (err) {
            oparam = param;   // if we have no param from parent - assume no parent and use param
          }
          if (oparam === null) oparam = param;  // null from parent param
          ...
          if (mmu || !already) {
            if (!qids[qqa.id]) {
              ...
              var containedqs = '';
              var master ="";
              var starred = oparam.contopt && oparam.contopt.share && oparam.contopt.share == "1" ? "starred" : "";
              starred = qqa.flags & SHARED ? "starred" : starred;   // flags overmaster options
              // TODO
              // flags used for quiz where it makes sense to change several with same value
              // all these quizzes are locked/hidden/shared
              // this will be moved back to opts when shift to 9.3+ with json for qtext
              if (starred) {
                if (qqa.parent && qqa.ptid == userinfo.id) {
                    continue;  // dont show if parent is the one we should copy
                }
                if (qqa.parent && sensei[qqa.parent]) {
                    continue;  // we already have listed a copy of this master
                }
                master = 'title="master copy"';
                qids[qqa.id] = 999;
                sensei[qqa.parent] = 1;   // ignore any further copies of this master
              }
              if (param.qlistorder) {
              ...
              // show star if this quiz is marked for sharing
              ...
              var qdiv = '<div title="'+tit+'" class="equest listqq '+statusclass+duup+'" id="zqq_'+qqa.id+'"><span class="qid">'
                         ...
                         + '<span '+master+' class="img img'+starred+'"></span><span title="'+qqa.name+'" > '
    ...
    for (var qiq in qids) {
      var qii = Math.min(totag,qids[qiq]);  // count of tags for question
  ...
  $j("#attachq").click(function() {
    var dia = ''
        + '<div id="selectag"><span class="tagtitle">Tags</span>'
        + '  <div id="chtag"></div>'
        + '  <div id="qqlist"></div>'
        + '</div>'
        + '<div id="multi" title="Insert original quiz - not a copy"> Master copy: <input id="master" name="master" type="checkbox"></div>';
    $j("#qlist").html(dia);
    var taggis = {};
    ...
    if (containername && containername.length) {
     ...
     $j("#qqlist").delegate(".equest","click", function() {
       ...
       var taglist = 'quizlist';
       $j.getJSON(mybase+'/log/getquesttags',{ tags:taglist, subject:subject }, function (qtlist) { selectQuestQuiz(qtlist) });
     return false;
  });
  $j("#attach").click(function() {
    ...
    $j.getJSON(mybase+'/log/gettags', { subject:subject }, function(tags) {
         ...
         mytags.sort();       // parent tags added after sorted list, need to resort
...
function editbind() {
        ...
        $j("#edcontrols").undelegate("#freedom","click");
        $j("#edcontrols").delegate("#freedom","click", function() {
        ...
        $j("#edcontrols").undelegate("#lockem","click");
        $j("#edcontrols").delegate("#lockem","click", function() {
           var tagged = $j("#sortable input:checked");
           var morituri = [];  // we who are about to die
           for (var i=0,l=tagged.length; i<l; i++) {
             var b = tagged[i];
           ...
           setFlags(morituri,LOCKED);
        });
        $j("#edcontrols").undelegate("#sharem","click");
        $j("#edcontrols").delegate("#sharem","click", function() {
           var tagged = $j("#sortable input:checked");
           var morituri = [];  // we who are about to die
           for (var i=0,l=tagged.length; i<l; i++) {
             var b = tagged[i];
           ...
           setFlags(morituri,SHARED);
        });
        $j("#edcontrols").undelegate("#hidem","click");
        $j("#edcontrols").delegate("#hidem","click", function() {
           var tagged = $j("#sortable input:checked");
           var morituri = [];  // we who are about to die
           for (var i=0,l=tagged.length; i<l; i++) {
             var b = tagged[i];
           ...
           setFlags(morituri,HIDDEN);
        });
        $j("#edcontrols").undelegate("#killem","click");
        $j("#edcontrols").delegate("#killem","click", function() {
           var tagged = $j("#sortable input:checked");
           var morituri = [];  // we who are about to die
           for (var i=0,l=tagged.length; i<l; i++) {
             var b = tagged[i];
        ...
        $j("#edcontrols").undelegate("#numem","click");
        $j("#edcontrols").delegate("#numem","click", function() {
           var tagged = $j("#sortable .equest input:checked");
        ...
        $j("#edcontrols").undelegate("#subem","click");
        $j("#edcontrols").delegate("#subem","click", function() {
           var tagged = $j("#sortable input:checked");
        ...
        $j("#edcontrols").undelegate("#numwe","click");
        $j("#edcontrols").delegate("#numwe","click", function() {
           var tagged = $j("#sortable .equest input:checked");
           if (tagged.length === 0) {  // non selected so do for all
               tagged = $j("#sortable .equest input");
           }
           var morituri = [];  // we who are about to be tagged with weeknumber
           for (var i=0,l=tagged.length; i<l; i++) {
             var b = tagged[i];
             var qid = $j(b).parent().attr("id").substr(3);
             morituri.push(qid); // question id
           }
           var tagname = wbinfo.trail[wbinfo.trail.length-1].name;
           var mm = tagname.match(/[0-9]+/);
           var numb = ((+mm[0] || 0) + 20) % 52;
           tagname = 'u' + (numb < 10 ? '0' : '') + numb;
           tagliate(morituri, tagname);
        });
        $j("#edcontrols").undelegate("#tagem","click");
        $j("#edcontrols").delegate("#tagem","click", function() {
           var tagged = $j("#sortable input:checked");
        ...
        $j("#edcontrols").undelegate("#rulem","click");
        $j("#edcontrols").delegate("#rulem","click", function() {
           var tagged = $j("#sortable input:checked");
        ...
        $j("#edcontrols").undelegate("#ownem","click");
        $j("#edcontrols").delegate("#ownem","click", function() {
           var tagged = $j("#sortable input:checked");
           var morituri = [];  // we who are about to be owned
           for (var i=0,l=tagged.length; i<l; i++) {
             var b = tagged[i];
        ...
        $j("#edcontrols").undelegate("#dupem","click");
        $j("#edcontrols").delegate("#dupem","click", function() {
           var tagged = $j("#sortable input:checked");
...
function rule(morituri) {
  // make questions parent - all siblings become children, original parent also
  // questions with no parent are ignored
  var given = [];
  ...
  for (var i=0,l=morituri.length; i<l; i++) {
    ...
    $j.post(mybase+'/log/editquest', { action:'owner', qid:qid }, function(msg){
        edqlist();
    });
  }
}
...
function clearFlags(morituri) {
  // remove flags for selected questions (most likely quiz)
  var given = [];
  if (morituri.length == 0) return;
  for (var i=0,l=morituri.length; i<l; i++) {
    var myid = morituri[i];
    var elm = myid.split('_');
    var qid = elm[0], instance = elm[1];
    $j.post(mybase+'/log/editquest', { action:'update', qid:qid, nuflags:0 }, function(msg){
        edqlist();
    });
  }
}
function setFlags(morituri,nuflag) {
  // set flags for selected questions (most likely quiz)
  var given = [];
  if (morituri.length == 0) return;
  for (var i=0,l=morituri.length; i<l; i++) {
    var myid = morituri[i];
    var elm = myid.split('_');
    var qid = elm[0], instance = elm[1];
    $j.post(mybase+'/log/editquest', { action:'update', qid:qid, flags:nuflag }, function(msg){
        edqlist();
    });
  }
}
function numriate(morituri) {
  // tagname assumed to be on the form "Week38" --> Week38,Week39,Week40 ...
  // week --> week0 week1 ..
  // week08 -> week08,week09,week10
  var given = [];
  if (morituri.length == 0) return;
  var tagname = $j("#tnavn").val() || '';
  if (!tagname) return;
  var maybeNum = tagname.match(/\d+/);
  var num = maybeNum ? maybeNum[0] : 0;
  var start = +num;
  var tagname = maybeNum ? tagname.replace(/\d+/,"€") : tagname+"€";
  for (var i=0,l=morituri.length; i<l; i++) {
    var myid = morituri[i];
    var elm = myid.split('_');
    var qid = elm[0], instance = elm[1];
    //var numname = (i+1)+'-'+tagname;
    var nunum = (''+(start+i)).length < num.length ? '0'+(start+i) : start+i;
    var numname = tagname.replace('€',nunum)
...
function tagliate(morituri,tagname) {
  var given = [];
  if (morituri.length == 0) return;
  if (!tagname) return;
  for (var i=0,l=morituri.length; i<l; i++) {
...
function workbook(coursename) {
    ...
    var studlist = elever ? elever.join(',') : '';
    wbinfo.weeksummary = showAweek(false,gru,elever,info,absent,wbinfo.timmy,tests,plandata,uke,tjd,section);
    $j.getJSON(mybase+'/log/workbook',{ courseid:wbinfo.courseid, coursename:coursename,studlist:studlist,subject:fag }, function(resp) {
        if (resp) {
          var courseinfo;
          try {
...
var qdefault = {  "quiz"     : {navi:1,fasit:0,hints:1,showpoints:1,omstart:1,adaptiv:1,antall:10 ,fiidback:"some",rank:1,karak:1,hintcost:0.10,attemptcost:0.1}
                , "lab"      : {darwin:0,navi:1,fasit:1,hints:1,showpoints:1,omstart:1,adaptiv:1,antall:50,fiidback:"lots",rank:1,karak:0,hintcost:0.0,attemptcost:0.0}
                , "homework" : {navi:1,fasit:1,hints:1,omstart:0,showpoints:1,adaptiv:1,antall:20,fiidback:"lots",rank:0,karak:0,hintcost:0.05,attemptcost:0.05}
                , "exam"     : {darwin:0,navi:0,fasit:0,hints:0,showpoints:0,omstart:0,adaptiv:0,antall:2 ,fiidback:"none",rank:0,karak:0}
               };
...
function eedit(myid,q,target) {
 var descript = { multiple:'Multiple choice', dragdrop:'Drag and Drop', sequence:'Place in order'
   ...
   $j("#saveq").click(function() {
        ...
        var nuflags = (+contopt.hidden * HIDDEN) | (+contopt.locked * LOCKED) | (+contopt.share * SHARED);
        ...
        $j.post(mybase+'/log/editquest', { action:'update', qid:myid, qtext:newqtx, name:qname, status:qstatus, avg:hard,
                nuflags:nuflags,  qtype:qtype, points:dialog.qpoints, subject:dialog.subject }, function(resp) {
   ...
   if (target == '#main') $j("#killquest").click(function() {
    ...
    function editVariants(q) {  // qu is a question
      ...
      switch(q.qtype) {
        case 'js':
           var perfect = (dialog.contopt.perfect != undefined) ? dialog.contopt.perfect : 0;
           var elements = {
                 defaults:{  type:"text", klass:"copts" }
               , elements:{
                    perfect:       {  type:"yesno", value:perfect }
                 }
               };
           var res = gui(elements);
           s += '<span id="inputdiv">'
              + ' <span title="Besvarelse må være feilfri for å få score">Feilfri  besvarelse {perfect}</span></span><p>';
           s = s.supplant(res);
           break;
        case 'abcde':
           var rekke = (dialog.contopt.rekke != undefined) ? dialog.contopt.rekke : 1;
           var optlist = drawABCDE(q.options,q.fasit);
           var elements = {
                 defaults:{  type:"text", klass:"copts" }
               , elements:{
                    rekke:       {  type:"yesno", value:rekke }
                 }
               };
           var res = gui(elements);
           s += '<span id="inputdiv">'
              + ' <span title="Spørsmål må besvares i rekkefølge">Rekkefølge {rekke}</span></span>';
           s = s.supplant(res);
           s += '<h3>Delspørsmål</h3>'
        ...
        case 'random':
           ...
           var beforetag = dialog.contopt.beforetag || '';
           ...
           var elements = {
               ...
               , elements:{
                   ...
                   , tags:         { value:usetags }
                   , beforetag:         { value:beforetag }
                   , demand:         { value:demand }
                 }
               };
           var res = gui(elements);
           s += 'Instillinger for random: <div id="inputdiv">'
             + '<div title="Velg fra kapitler til og med denne (f.eks kap5, Kapittel8.3)">Kapittel til og med {beforetag}</div>'
        ...
        case 'quiz':
           ...
           var hidden = q.flags & HIDDEN ? 1 : ((dialog.contopt.hidden != undefined) ? dialog.contopt.hidden : 0);
           var fasit = (dialog.contopt.fasit != undefined) ? dialog.contopt.fasit : 0;
           var darwin = (dialog.contopt.darwin != undefined) ? dialog.contopt.darwin : "";
           ...
           var hintcost = dialog.contopt.hintcost || '0.1';
           ...
           var karak = (dialog.contopt.karak != undefined) ? dialog.contopt.karak : 1;
           var rank = (dialog.contopt.rank != undefined) ? dialog.contopt.rank : 1;
           var fiidback = (dialog.contopt.fiidback != undefined) ? dialog.contopt.fiidback : 'some';
           ...
           var omstart = (dialog.contopt.omstart != undefined) ? dialog.contopt.omstart : 1;
           ...
           var showpoints = (dialog.contopt.showpoints != undefined) ? dialog.contopt.showpoints : 1;
           ...
           var share = (dialog.contopt.share != undefined) ? dialog.contopt.share : 0;
           var adaptiv = (dialog.contopt.adaptiv != undefined) ? dialog.contopt.adaptiv : 1;
           var elements = {
               ...
               , elements:{
                 ...
                 , darwin:        {  type:"select", klass:"copts",  value:darwin, options:[{ value:""},{ value:"darwin"},{ value:"easy"},{ value:"medium"},{ value:"hard"} ] }
                 , hints:         {  type:"yesno", value:hints }
                 , showpoints:    {  type:"checkbox",  value:showpoints }
                 ...
                 , share:         {  type:"checkbox", value:share }
                 ...
                 , start:         {  klass:"copts pickdate", type:"date", value:start }
                 , stop:          {  klass:"copts pickdate", type:"date", value:stop }
                 , fiidback:      {  type:"select", klass:"copts",  value:fiidback, options:[{ value:"none"},{ value:"some"},{ value:"lots"} ] }
                 , skala:         {  type:"select", klass:"copts",  value:skala, options:[{value:"plain"},{ value:"medium"},{ value:"easy"},{ value:"hard"} ] }
           ...
           s += '<h4>Instillinger for prøven</h4> <div id="inputdiv">'
             ...
             + '<div class="underlined" title="Deles med andre teachers">Deling {share}</div>'
             ...
             + '<div class="underlined" title="Skal poeng for hvert svar vises">Vis poeng{showpoints} </div>'
             ...
             + '<div class="underlined" title="Karakterskala som skal brukes, '
             +              'plain gir (1,2,3,4,5,6), easy for en lett prøve (streng vurdering), hard gir snill vurdering">Skala {skala}</div>'
             ...
             + '<div class="underlined" title="Velg vanskegrad, sprsml kan bruke dette til å gjøre seg enkle...vrine, darwin prøver å treffe elevens nivå">Nivå {darwin}</div>'
...
wb.render.normal  = {
       ...
       , editql:function(questlist,wantlist) {
            ...
            var seenQuiz = false;   // we have only seen questions
            var scorecount = 0;     // count questions with valid difficulty score
            var scoresum = 0;       // sum up avg score for those valid
            for (var qidx in questlist) {
              qu = questlist[qidx];
              if (qu.qtype == 'quiz') seenQuiz = true;
              ...
              if (qu.count > 4) {
                  // sufficient number of responses to use avg score
                  scorecount++;
                  scoresum += +qu.avg;
              }
            ...
            if (wantlist) {
              ...
              qq = qql.join('');
              if (qq == '') {
                qq = '<div id="sortable">' + qq + '</div><div id="edcontrols"><br>';
                if (wbinfo.trail == '') {
                ...
                qq = '<div id="sortable">' + qq + '</div><div id="edcontrols"><br>';
                if (scorecount > 0) {
                    var expectavg = +(scoresum/scorecount);
                    qq += '<div class="expect">Forventa score basert på ' + scorecount + ' spørsmål er ' + expectavg.toFixed(2) + '</div>';
                }
                ...
                qq += '<span title="Frigjør fra original, dette blir en uavhengig kopi" class="listbut" id="ownem">Eierskap</span>';
                qq += '<span title="Overta ansvar for alle kopier, dette blir malen" class="listbut" id="rulem">Ansvar</span>';
                qq += '<span title="Nummerer valgte sprsml" class="listbut" id="numem">Nummerer</span>';
                qq += '<span title="Sett uketag for valgte" class="listbut" id="numwe">Uketag</span>';
                ...
                if (seenQuiz) {
                  qq += '<span title="Skjul valgte for elever" class="listbut" id="hidem">Skjul</span>';
                  qq += '<span title="Del valgte quiz med andre lærere" class="listbut" id="sharem">Del</span>';
                  qq += '<span title="Lås valgte quiz - prøve ferdig" class="listbut" id="lockem">Lås</span>';
                  qq += '<span title="Fjern lås/del/skjul" class="listbut" id="freedom">Frigi</span>';
                  qq += '<div class="clearbox"></div>';
                }
              }
              if (wbinfo.haveadded < 2) {
                ...
                qq += '<p class="bigf">For å redigere setter du markøren over et spørsmål og klikker på blyanten.'
                   ...
                   +  '<li>extend - legg nytt spørsmål til påbegynt quiz</li><li>addQuiz - legg til eksisterende quiz</li>'
                   +  '<li>sync - synkroniser mot originaler </li>'
                   +  '</ul></p>';  // '
       ...
       , qrend:function(contopt,iid,qid,qua,qrender,scorelist,callback) {
         // renderer for a single question
              ...
              var scored = qua.attemptnum !== undefined ? 1 : 0;
              var qdiv = wb.render.normal.displayQuest(qua,iid,contopt,sscore,scored);
              var sum = 0;
       ...
       , qlist:function(container,questlist,contopt, callback) {
         // renderer for question list
            ...
            $j.post(mybase+'/log/renderq',{ container:container, questlist:questlist }, function(qrender) {
              ...
              if (contopt && contopt.antall && +wbinfo.page[wbinfo.containerid] >= 0) {
               // paged display
              ...
              for (var qi=qstart; qi < qant; qi++) {
                ...
                if (qu) {
                    ...
                    console.log("QRENDER ERR:",qrender,qi,qstart,qant,contopt);
                    qql.push('<div class="question qq'+qi+'">Klikk på breadcrumb</div>');
                }
              }
         ...
         , displayQuest:function(qu,qi,contopt,sscore,scored,fasit) {
              ...
              // (we display ungraded questions on first show of question)
                ...
                var displayfasit = '';
                var status = qu.status;
                var inorder = '';  // changed in abcde if parts must be answered one by one
                ...
                switch(qu.qtype) {
                      case 'quiz':
                          ...
                          var embellish = '';
                          if (qu.flags & HIDDEN || (mycopt && mycopt.hidden == "1") )  {
                            if (!teaches(userinfo.id,wbinfo.coursename)) {
                               return '';
                            }
                            embellish += " cloaked";
                          ...
                          if (mycopt && mycopt.start) {
                            ...
                            start = new Date(elm[0],+elm[1]-1,elm[2]);
                            if (mycopt.hstart) {
                          ...
                          if (mycopt && mycopt.stop) {
                            ...
                            stop = new Date(elm[0],+elm[1]-1,elm[2]);
                            if (mycopt.hstop) {
                          ...
                          if (qu.flags & LOCKED || mycopt && mycopt.locked == "1") {
                            embellish += " locked";
                          }
                      ...
                      case 'abcde':
                          ...
                          if (!param.donotshow && param.options && param.options.length) {
                              ...
                              for (var i=0, l= param.options.length; i<l; i++) {
                                ...
                                // TODO some confusion about enabled, changed from string to boolean
                                // TODO causes string true to be placed after class="xx" true
                                var enabled = (scored +attempt == i || (!scored && i == 0 && attempt==0)) ? '' : ' readonly="readonly"';
                                inorder = ' inorder';
                                if (param.contopt && param.contopt.rekke == 0) {
                                    // override for forcing user to answer in order
                                    enabled = true;
                                    inorder = '';
                                }
                                ...
                                if (question.substr(0,2) === '::') {
                                    question = question.substr(2);
                                    if (i > attempt + scored) {
                                      // dont show question text before my turn if starts with '::'
                                      question = '...';
                                    }
                                }
                                var guidance = parts[1] || '';
                      ...
                      case 'jscore':
                      case 'fillin':
                          var inptype = "text";   // default type of input
                          if (qu.feedback && qu.feedback != 'none' ) {
                            if (/^[0-9-]+$/.test(qu.feedback)) {
                              checkmarks = qu.feedback.split('');
                              qu.feedback = '';
                          ...
                          adjusted = adjusted.replace(/(_?&nbsp;&nbsp;&nbsp;&nbsp;)/g,function(m,ch) {
                                ...
                                if (ch.substr(0,1) == '_') {  // [[anytext]] will be repaced by textarea - not input text
                                  ...
                                  var chk = klasslist.length ? 'class="'+klasslist.join(' ')+'"' : '';
                                  var ffy = (ff) ? '<span class="fasit gui">'+unescape(ff)+'</span>' : '';
                                  ret = '<span '+subclass+'><input '+chk+subus+' type="'+inptype+'" value="'+vv+'" /></span>'+ffy;
                                }
                                iid++;
                                return ret;
                              });
                  ...
                  var reloadme = '';
                  if (contopt.omstart && contopt.omstart == "1") {
                      reloadme = '<span class="reloadme">&nbsp;</span>';
                  }
                  var hardnes = (qu.avg == 0) ? '' : (qu.avg < 0.5) ? 'hard' : (qu.avg < 0.8) ? 'medium' : 'easy';
                  qtxt = '<span class="qnumber '+hardnes+'">Spørsmål '+qnum + qname + ' ' + qu.points + 'p' + reloadme + '</span>' + qtxt+displayfasit;
                  ...
                  return '<div class="question '+statusclass+inorder+' qq'+qi+'" id="qq'+qu.qid+'_'+qi+'">' + hints+ qtxt + studnote + '</div>';

                  function decoration() {
                     if (scored || (attempt != '' && attempt > 0)) {
                       qtxt += '<span id="at'+qi+'" class="attempt gui">'+(attempt)+'</span>';
                     }
                     if (contopt.showpoints !== "0" && (scored || attempt > 0 || score != '')) {
