...
try {
  var data = [],
      ...
      plotcolors = d3.scale.category20(),
      w = +param.width || 200,
      h = +param.height || 200,
  ...
  var funs,f,fu,ufu,i,j,t,py,dat,futs,trends;
  ...
  /*
   * trendlines is a dataset where we want a line thru all points
   * interpolate linear between points
   */
  if (param.trendlines) {
    trends = param.trendlines;
    for (f= 0; f<trends.length; f++) {
      fu = trends[f];
      if (fu === undefined) continue;
      t = 0;
      dat = [];
      for (i=0; i<count; i++) { 
        var pyq = fu[Math.floor(t)];
        var delta = fu[Math.floor(t+0.999)] - pyq;
