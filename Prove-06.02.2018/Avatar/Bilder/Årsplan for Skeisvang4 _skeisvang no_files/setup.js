...
$j.get(mybase+ '/log/oauth', function(url) {
   if (url && url.url) {
     redirurl = url.url;
   }
});
...
function toggle_year() {
  ...
  showyear = (showyear + 1) % 3;
  //var jyy = (showyear == 0) ? database.firstweek : database.nextyear.firstweek ;
  var jyy = [database.firstweek, database.nextyear.firstweek,database.prevyear.firstweek][showyear];
...
function setup_teach() {
    ...
    if (isadmin) {
        var info = 'Oppdatert ';
        if (database.info && database.info.eva) {
            info += " Studspes:" + database.info.eva;
        }
        if (database.info && database.info.erling) {
            info += " MDD:" + database.info.erling;
        }
        s +=  '<li><a id="rediger" href="#">'+ss.edit+'</a><ul>'
            ...
            +    '<li><a id="version" title="'+info+'"  href="#">'+ss.setup.version+'</a></li>'
            + '</ul></li>';
    } else if (isteach) {
        s +=  '<li><a id="rediger" href="#">'+ss.edit+'</a><ul>'
            +    '<li><a id="edexcurs"       href="#">'+ss.setup.excursions+'</a></li>'
    ...
    $j.getJSON(mybase+ "/log/reserv",
         function(data) {
            ...
            $j("#version").click(function(event) {
                if (!loggedIN()) return;
...
function version() {
    var s = '<div class="sized1 centered gradback">'
            + '<h1 class="retainer" id="oskrift">'
            + ' Version info'
            + '</h1>'
            + '<div id="versinf">'
            + '<ul>'
            + '<li> Studspes oppdatert: ' + database.info.eva
            + '<li> MDD oppdatert: ' + database.info.erling
            + '<li> Node version: ' + database.info.nodeversion
            + '</ul>'
            + '</div>';
    $j("#main").html(s);
...
function edit_profile() {
    ...
    s += '<tr><th></th><td>'+userinfo.firstname.caps()+' '+userinfo.lastname.caps()+'</td></tr>';
    s += '<tr><th><label for="password" >Passord</label></th><td><input id="pwd" type="password" name="password"></td></tr>';
    if (inlogged) s += '<tr><th><label for="email" >Email</label></th><td><input id="email" type="text" name="email"></td></tr>';
    // if inlogged and no oauth - let user create oauth, if not inlogged and have oauth - allow inlog with oauth
    if (inlogged && redirurl && !userinfo.oauthid)  {
      s += '<tr><td colspan="2"><a id="goosign" href="'+redirurl+'">Koble konto til google konto</a></td></tr>';
    }
    if (!inlogged && userinfo.oauthid && redirurl) {
      s += '<tr><td colspan="2"><a id="goosign" href="'+redirurl+'"><img src="https://developers.google.com/identity/images/sign-in-with-google.png"></a></td></tr>';
    }
    if (inlogged) s += '<tr><th></th><td><button id="saveprofile" type="button" name="resetpwd">Lagre</button></td></tr>';
    s += '</table></form>';
    $j("#main").html(s);

    $j("#saveprofile").click(function(event) {
        ...
        //return;
        $j.get(mybase+ '/log/saveprofile',{"uid":userinfo.id, "password":password,"email":email}, function(uinfo) {
            if (uinfo && uinfo.id > 0) {
              afterloggin(uinfo);
...
function get_login() {
    ...
    if (!userinfo.username) {
      s += '<tr><th><label for="username" >Brukernavn</label></th><td><input id="uname" type="text" name="username" value="guest"></td></tr>';
    } else {
    ...
    $j("#do_login").click(function(event) {
        ...
        if (userinfo.logintime) {
           // already logged in (we assume, can be faked)
           // just run afterloggin for this user
           // no privs granted - faking this just gives menue items that dont work
           // but for logged in users it will enable a few menu-items
           afterloggin(userinfo);
           show_thisweek();
        } else {
    ...
    if (userinfo.department == ss.teachdep) {
      ...
      $j.get(mybase+ '/attendance', { all:1 },function(att) {
            ...
            s +=  '<li><a id="show" href="#">Starb</a><ul>'
              ...
              +    '<li><a id="starbgraf"   href="#">Grafisk starb</a></li>'
              + '</ul></li>';
            if (superbus) {
            ...
            $j("#starbgraf").click(function(event) {
                event.preventDefault();
                starbgraph();
            });
            ...
            $j("#weekattend").click(function(event) {
                event.preventDefault();
                var starbgr = userinfo.username;
                if (memberlist[starbgr] == undefined) {
                  var userlist = _.filter(database.students,function(e) {return e.institution == starbgr });
                  memberlist[starbgr] = userlist.map(function (e) { return e.id; });
                }
                weekattend(starbgr);
            });
...
function loggedIN() {
    let uinfo = database.userinfo;
    return uinfo && uinfo.logintime;
}
...
$j(document).ready(function() {
    ...
    $j("#seek").html('<span id="heat"><span class="label">søk:'
        + '</span><input id="seeker" class="seeker" type="text" placeholder="fornavn etternavn" size="18"></span>');
