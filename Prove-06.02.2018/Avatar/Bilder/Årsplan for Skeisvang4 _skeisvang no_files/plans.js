...
function synopsis(coursename,plandata,tests) {
  ...
  for (section in  plandata) {
    ...
    var solo = [];      // singular test for a few studs
    for (var j=0; j<5; j++) {
        var pro = prover[jd+j];
        var title = [];
        var testitle = [];
...
function show_all(thisweek,options) {
    ...
    if (typeof(options) == "undefined") {
       options = 0;
       if (isadmin) options |= 8;
       if (isteach) options |= 16;
    }
    ...
    var teachevent = (options & 16) ? 'checked="true"' : '';
    var events = database.aarsplan;
    var prover = alleprover;
    s = '<div class="centered sized1"><div class="gui" id="editmsg">Kryss av for å vise hd og prøver.'
         ...
         + (isteach ? '<label for="useteach" class="adliste">Teach notater <input id="useteach"'+teachevent+' type="checkbox"></label>' : '')
    ...
    var tfooter ="</table>";
    s += theader;
    var start = [database.firstweek, database.nextyear.firstweek,database.prevyear.firstweek][showyear];
    var stop = [database.lastweek, database.nextyear.lastweek,database.prevyear.lastweek][showyear];
    start = (showyear == 0) ? thisweek  : start;
    // skip passed weeks if we are showing current year
    ...
    for (i= start; i < stop; i += 7) {
      ...
      for (j=0;j<6;j++) {
        ...
        if (database.freedays[i+j]) {
          ...
          var admcansee = (infotxt.admin && isadmin && adminevent ) ? ('<span class="redfont">'+infotxt.admin+'</span>') : '';
          var teachcansee = (infotxt.teach && isteach && teachevent ) ? ('<span class="bluefont">'+infotxt.teach+'</span>') : '';
          txt = anycansee + admcansee + teachcansee + xtra;
        }
...
function starbgraph(cutof) {
  var cutof = cutof ? cutof : 17;
  // first find days with starb (more than 20)
  var starbdays = Object.keys(allattend.daycount);
  // pick out 60 last good entries
  var good = (starbdays.filter(function(jd) { return +allattend.daycount[jd]> 20} ))
                   .sort( function(a,b) { return a-b; } ).slice(-60);
  // build a trend for each room
  var rids = (Object.keys(allattend.rooms)); 
  var trends = [];
  var max = 0;
  var len = 0;
  var mynames = database.roomnames;
  var rnames = ["30elever"];
  var roomkeys = [];
  var dosums = document.querySelector("#summer:checked") ? true : false;
  var wantrooms = document.querySelectorAll("input.romchk:checked");
  var hotrooms = (Array.from(wantrooms)).map(function(e) { return e.value;} );
  for (var r of rids) {
    var t = [];
    var ro = allattend.rooms[r];
    var sum = 0;
    var extreme = false;  // ignore extremes
    var mymax = max;
    for (var jd of good) {
      var ant = ro[jd] ? ro[jd].length :  0;
      t.push(ant);
      sum += ant;
      //if (ant > 90) { extreme = true; continue; }
      if (ant > mymax) mymax = ant;
    }
    if (hotrooms.length > 0) {
       if (hotrooms.includes(r)) {  // we want to see this room
         trends.push(t);
         max = mymax;
         len = Math.max(len, t.length);
         rnames.push(database.roomnames[r]);
         roomkeys.push(r);
       }
    } else if (!extreme && t.length && sum / t.length > cutof ) {
      // only view rooms with more than cutof average
      trends.push(t);
      max = mymax;
      len = Math.max(len, t.length);
      rnames.push(database.roomnames[r]);
 ...
 if (dosums) {
   var summation = trends.reduce( function(sum,e) { 
     e.forEach( function(n,i)  { 
       if(sum[i]) { sum[i] += n;} else { sum[i] = n;} 
      } ); 
      return sum; 
    }, []);
   trends.push(summation);
   rnames.push('TOTAL');
   max = Math.max(...summation);
 }
 max = Math.max(max,30);
 var roomchk = (roomkeys.map(function(r) { return '<label>'+database.roomnames[r]
           + '<input class="romchk" type="checkbox" value="'+r+'"></label>'; } )).sort().join(' ');
 var s = '<div class="graph">';
 s += '</div>Snitt &ge; <input id="cutof" type="number" value='+cutof+'><button type="button" id="stab">Replot</button>';
 s += '<div>' + roomchk + ' <br>Summer<input id="summer" type="checkbox"></div>';
 $j("#main").html(s);
 lineplot({target:"#main",fu:[function(t) { return 30}],xrange:[0,len],
             trendlines:trends, labels:rnames.join(","),
             drawline:true, interpolate:true, 
             xnames:good.map( function(e) { var d = julian.jdtogregorian(+e); return d.day+":"+d.month;} ),
             yrange:[0,max+5],height:550,width:1024 }); 
  $j("#stab").click(function(event) {
      starbgraph($j("#cutof").val());
  });
}
