...
function findFreeTime() {
  ...
  $j.getJSON(mybase+ "/log/getmeet", function(data) {
    ...
    var remap ={ AvdLeder:{field:'institution',
                 map:{ 'Realfag':'Berit', 'Samfunnsfag':'Eva', "Filologi":"Eva", 'Bibliotekar':"Eva",'Kontoret':'Mona',
                       'Musikk':'Erling', 'DansDrama':'Ruth', "Språk":'Ruth','Naturfag':'Knut-Oskar','IT':'Lest','Admin':'Kari'} }};
    ...
    var freeTimeTable = function (userlist,roomid,delta) {
      ...
      t += '<div id="duration">'+ss.meet.duration+' : <span class="dur">10</span> <span class="dur">20</span> <span class="dur">30</span> <span class="dur heck2">40</span>'
         + ' <span class="dur">60</span> <span class="dur">80</span><div id="dato">'+meetdate+'</div>'
      ...
      $j("#makemeet").click(function() {
         ...
         var day = minfo.slotz[0].split('_')[0];
...
function myMeetings(meetid,delta) {
  ...
  delta = typeof(delta) != 'undefined' ?  +delta : +deltamemory;    // week offset from current date
  $j.getJSON(mybase+ "/log/getmeet", function(data) {
    meetings = data.meetings;
        ...
        if (mee[userinfo.id]) {
          for (var mmid in mee[userinfo.id]) {
            ...
            var meetdiv = '<div  class="meetlist'+active+' acc'+abba.klass+myown+'"><span class="meetinfo">'
                          ...
                          +'</span><span class="meetdato">' + meetime + ' ' + romdager[day%7]+' '
    ...
    if (meet) {
      if (owner) {
        ...
        s+= '<div id="controls">'
            ...
            + '<div id="resend" class="meetbutton">Resend (FAUX)</div>'
            + '</div>';
        s+= "</div>";
      } else {
      ...
      var meetime =  meetTimeStart(meet.julday+meet.day,[meet.day+'_'+meet.slot],0,5*meet.dur);
