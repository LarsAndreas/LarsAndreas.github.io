...
function rom_reservering(room,delta,makeres) {
    ...
    if (database.roominfo[room]) {
      ...
      if (restrict.length > 0) {
        ...
        // allow members of given departments to make bookings
        if ($j.inArray(database.userinfo.department,restrict) >= 0) {
          makeres = true;
        }
      }
    }
                    ...
                    if (res.name == room) {
                        ...
                        if (res.eventtype == 'hd') {
                          ...
                          timetable[res.day][rslot] = '<div id="'+res.id
                              +'" class="resme '+kkla+' rcorner gradbackgreen textcenter"><span class="edme">' + res.value + '</span><div class="killer"></div></div>';
                        } else {
